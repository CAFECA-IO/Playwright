...
test('example test', async ({ page, extensionId  }) => {
  ...
  await expect.soft(page.locator("#app-content > div > div.mm-box.main-container-wrapper > div > div > div > ul > li:nth-child(3) > button")).toHaveText("Import an existing wallet");
  await page.locator("#app-content > div > div.mm-box.main-container-wrapper > div > div > div > ul > li:nth-child(3) > button").click();
  
});

test('popup page', async ({ page, extensionId }) => {
  await page.goto("chrome-extension://oimbidjhffkibgpbooehfgphglkggckb/popup.html");
